...
const UserFin = () => {
  ...
  const handleSubmitChange = () => {
    const a = document.getElementById("submit-display")
    a.innerHTML = "Be Conservative"
    a.style.color = "black"
      ...
      <div className="max-w-4xl mx-auto p-6 bg-gradient-to-r from-slate-400 to-slate-400 rounded-lg shadow-md" >
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="col-span-full mt-4">
            <button type="submit" onClick={handleSubmitChange} className="bg-black text-white px-4 py-2 rounded-md hover:bg-blue-600">Submit</button>
          </div>
        </form>
        <div id="submit-display" className='mt-10 p-3 rounded-lg'>
        </div>
      </div>
    </div>
  );
